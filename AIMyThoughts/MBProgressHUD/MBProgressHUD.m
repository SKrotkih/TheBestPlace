...
+ (void) startProgressWithAnimation: (BOOL) animated
{
    UIView* window = [AIAppDelegate sharedDelegate].window;

    if (_hud == nil)
    {
        _hud = [[self alloc] initWithView: window];
    }
...
+ (void) stopProgressWithAnimation: (BOOL) animated
{
    if (_hud)
    {
        _counter--;
